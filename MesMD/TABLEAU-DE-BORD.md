...
| 64 | **Cible (à détailler par l'utilisateur) : l'IA aide l'admin à proposer le bon PDF officiel** au dépôt d'un référentiel — l'admin garde le dernier mot (jamais l'IA seule, cap). Aujourd'hui l'admin fournit lui-même le PDF (lien/dépôt). Étape la plus sensible (on source la vérité) : recherche réelle, jamais inventer. Séparé du découpage. CLAUDE.md 656 marquée « cible ». | à cadrer | à scorer | à scorer | à scorer | RAG / Référentiel — sourcing PDF | — |
