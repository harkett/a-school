...
> - **Sens 2 — l'admin propose lui-même sa règle de découpe, le dev vérifie et valide.** Pas encore branché (aujourd'hui : le dev propose la règle, l'admin ne fait que Valider/Rejeter). La colonne `regle_depose_par` (règle EN BASE, `referentiels`, migration #3 — plus de fichier `regle-decoupe.json`) anticipe déjà les deux sens.
> - **Arbitrage du flou par l'admin — MÉCANISME FAIT (09/07, migration #4).** L'admin **tranche** désormais la tranche d'âge d'un cas flou (boutons d'options + `validerAge` → `POST /arbitrage-flou`), écrit **EN BASE** (`referentiels.arbitrage`), relu par la fiche à l'ingestion. Plus aucun mapping flou en dur. Reste (WORK étape 7) : l'admin arbitre les **vrais** flous crèche puis ingestion.
...
| [45](#item-45) | Multi-fournisseurs IA — failover / tableau de bord / routage auto *(08/07 : 1er pas fait — la combo admin « Fournisseur » affiche désormais TOUS les fournisseurs, non-opérationnels grisés « pas encore disponible » ; bascule réelle vers une 2e IA = chantier gelé, en attente clé+modèle)* | à scorer | à scorer | à scorer | à scorer | FUTUR / Infra IA — à cadrer | — |
...
| 66 | **Question d'archi — où vivent `regle-decoupe.json` / `arbitrage-flou.json` ?** Aujourd'hui en fichiers à côté du PDF ; frotte avec le cap « tout en base » (données saisies par l'admin à l'exécution, hors sauvegarde base). À trancher : fichiers (matière première versionnée) vs table reliée à `referentiels`. Non urgent, réversible (accès isolé : `_charger_arbitrage`, `_regle_decoupe_path`). Le PDF, lui, reste un fichier. Garde-fou en vigueur : règle CLAUDE.md « donnée métier en fichier → prévenir AVANT ». | à cadrer | à scorer | à scorer | à scorer | ARCHI / Données — question ouverte | — |
| 68 | **Lycée : 3 → 6 niveaux (dédoubler générale / technologique)** — base actuelle = 3 niveaux fusionnés, voie **générale** seulement (Seconde pré-réforme) ; cible = Seconde G/T + Première G/T + Terminale G/T ; BO unique déposé dans chaque dossier, l'IA constitue le référentiel par niveau (générique cycle+niveau). Chantier de **structure** (migration + remap matières via procédure standard) ; miroir avant vraie base. **En attente** — on y revient le jour où on gère le cycle Lycée. | à cadrer | à scorer | à scorer | à scorer | STRUCTURE / Niveaux Lycée — à cadrer | [D68](BOUSSOLE/D68.md) |
| 69 | **Analyse amont IA — qualité / déterminisme du jugement** (né du branchement item 67). Observé le 10/07 en live : l'IA (Groq, température 0) flague un **nombre de doutes VARIABLE** d'un run à l'autre (4 puis 0 sur le même document crèche). L'**architecture est déjà robuste** (verdict figé une fois, relu des deux côtés → jamais de divergence — item 67) ; ce qui reste = la **stabilité/qualité du jugement** lui-même : durcir le **prompt d'analyse amont** (critères explicites de ce qui est « douteux », few-shot), voire **plusieurs passes + vote**. **Indépendant de l'archi.** | à cadrer | à scorer | à scorer | à scorer | RAG / Analyse amont — qualité prompt | — |
| ~~71~~ | ~~**Refus du filtre → modale bloquante admin**~~ **FAIT 10/07** : cartouche « **Résultat du filtre par niveau** » (sous le découpage) = verdict du VRAI filtre (vert « prêt » / rouge « N cas à trancher ») + **modale bloquante** à l'ouverture d'un couple en échec (règle « incohérence = modale »), qui oblige l'admin à revenir trancher. **Indépendant de l'item 44** (branché sur l'aperçu, pas sur l'ingestion). Idée Harketti 10/07, livrée le jour même. | 1 session | ★★★★☆ | ★★★★★ | **fait** | RAG / Ingestion — garde-fou UX | — |
| 70 | **Nom du produit « aSchool » en réglage admin (en base, injecté partout)** — aujourd'hui « aSchool » est écrit EN DUR partout (objet + signature des mails, en-têtes, écrans). Idée (Harketti, 10/07, née de l'objet du mail TEMPS 2) : en faire UN paramètre unique réglable dans l'admin, stocké EN BASE, injecté à chaque endroit où le nom apparaît → on le change à un seul endroit, il se met à jour partout (cap « tout en base, rien en dur »). Chantier TRANSVERSAL : recenser TOUTES les occurrences (UI + mails + titres) et les remplacer d'un coup ; rien codé maintenant. | à cadrer | à scorer | à scorer | à scorer | INFRA / Branding — nom en réglage | — |
...
- [x] **« Tout en base » — 5 données métier migrées fichier/localStorage/dur → BASE** | 09/07/2026 (miroir, vraie base non touchée)
  *Règle absolue gravée : toute donnée métier vit en base (CLAUDE.md + mémoire `feedback_donnee_fichier_prevenir`). **5 migrations, du plus sûr au plus risqué, chacune prouvée sur `aschool_creche_miroir`** : **#1** seuil RAG `SCORE_MIN` (dur) → `referentiels.score_min` (`1f5f22e`) ; **#2** niveau prof (doublon `localStorage.aschool_niveau`) → base seule `users.niveau` + purge clé (`a616d5c`) ; **#5** matières candidates (`matieres-candidates.json`) → table `matieres_candidates` (`0c0d726`) ; **#3+#4** règle de découpe + arbitrage des cas flous (`regle-decoupe.json` / `arbitrage-flou.json`) → colonnes `referentiels.regle_*` + `.arbitrage` ; contrat d'ingestion refait (`fiche.charger_regle(ref)` par le socle avant `extract_pages`, plus aucune lecture fichier) ; 3 JSON crèche versés en base par script éphémère puis supprimés (`7a04486`). **Preuves :** suite complète **188 tests verts** + `apercu_decoupage('bebes_0_1_an')` tourne fichiers supprimés (27 unités, 18 Bébés). Chaîne Alembic 1 tête (`a3c4d5e6f7a8`). Doctrine alignée (CLAUDE.md, mémoire, REPRISE, D60, commentaires code/frontend). **Vraie base `aschool` intacte** — les 5 migrations s'y appliqueront à la bascule, sur GO. **Reste (étage 2) :** test visuel admin/prof.*

- [x] **Compte démo + activités d'exemple en dur — ÉRADIQUÉS (code + base dev)** | DEV 30/06 — en attente commit (GO)
  *Décision : abandon total du compte démo. **Code** : `backend/seed_exemples.py` supprimé (`git rm`, 3255 lignes : compte `demo@aschool.fr` + ~80 activités d'exemple en dur + son lancement `__main__`) ; appel + `try/except` retirés de `backend/main.py` (ex-lignes 110-120). **Frontend (suppression sèche)** : section « Les exemples de la Bibliothèque » de `Aide.jsx` retirée, astuce `AdminAide.jsx` (compte demo@aschool.fr) retirée, `MonReseau.jsx` nettoyé (strip `[Exemple]` + badge « Exemple » `partagee_par === 'Équipe aSchool'`). **Doc** : réserve `seed_exemples.py` retirée de `AUDIT-EN-BASE-VS-EN-DUR.md`. **Base dev** : script ÉPHÉMÈRE (créé → lancé → supprimé) a effacé les **71** activités + le compte démo (`users.id=1`) → prouvé **0** compte démo / **0** activité `[Exemple]` / **0** user / **0** activité en base. **PROD non touchée** (tâche séparée : le seed y tournait à chaque démarrage). Refonte de l'aide laissée à son chantier `TRACKER-AIDE.md`.*
