...
| ☐ | **67 — Détection d'ambiguïté par l'IA (applique la règle `CLAUDE.md`)** : remplacer la détection en dur (`_age_est_flou` + repli de `_age_bands`, `creche_0_3_ans.py`) par une **analyse amont par l'IA** (`generate()`) — l'IA lit le document, en tire elle-même la règle de classement, range le net, ne remonte que les vrais doutes ; l'admin valide. **Générique** (socle, pas la fiche crèche). ⚠️ Ne **PAS** supprimer le code en dur avant que l'IA le remplace (il alimente `apercu_unites` + l'ingestion → l'app casserait). **Pas 1 fait (10/07) : brique isolée testée (9 tests moqués + 4 preuves de généricité Groq — crèche/BTS/cycle 4/Première GT) ; restent pas 2 (neutraliser la surface + mapper vers matières canoniques) et pas 3 (brancher + retirer `_age_est_flou`).** | règle « L'ambiguïté se détecte par l'IA » dans `CLAUDE.md` |
