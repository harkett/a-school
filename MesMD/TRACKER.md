...
| ☐ | **56 — BTS CIEL Option A : artefact pré-réforme — RETIRÉ le 08/07/2026** : le référentiel + ses **236 chunks** + la fiche Python du BTS + le dossier ont été supprimés (prouvé sur le miroir ; SQL à rejouer sur la vraie base) ; **structure cycle/niveau BTS conservée**. Le BTS sera régénéré via la procédure standard (comme la crèche) → **plus rien à réconcilier** (à clore). | détail → [D56](BOUSSOLE/D56.md) |
...
| 🔄 | **67 — Détection d'ambiguïté par l'IA (applique la règle `CLAUDE.md`)** : remplacer la détection en dur par une **analyse amont par l'IA**. **Pas 1 fait** (brique isolée : 9 tests moqués + 4 preuves de généricité Groq — crèche/BTS/cycle 4/Première GT). **Pas 2/3 faits (10/07, commit `93ad18b`)** : `_age_est_flou` **supprimé** ; le doute est décidé par l'IA, verdict calculé **UNE fois** puis figé en base (`referentiels.doutes_ia`) et relu par l'**aperçu ET l'ingestion** (même verdict, jamais deux appels divergents ni rate-limit) ; l'arbitrage admin **prime** sur le parsing. Prouvé : **39 tests + preuve live sur le miroir**. **Restent :** ton **test étage 2** (déposer crèche → voir doutes → trancher → remplir) pour clore ; + qualité/**déterminisme** du jugement IA = **item 69** (séparé). | règle « L'ambiguïté se détecte par l'IA » dans `CLAUDE.md` |
| ☐ | **68 — Lycée : passer de 3 à 6 niveaux (dédoubler générale / technologique)** : en base, le cycle « Lycée » (id 4) n'a que **3 niveaux fusionnés** — Seconde (13), Première (14), Terminale (15) — rattachés aux seules matières de la **voie générale** (la Seconde porte même des libellés **pré-réforme 2019** : « Enseignements d'exploration », « Sciences »). Or la **voie** décide des matières et des programmes → elle fait partie de l'identité du niveau. Cible = **6 niveaux** : Seconde G/T, Première G/T, Terminale G/T. Méthode **générique cycle+niveau** : le BO officiel unique est déposé dans **chacun des 6 dossiers**, l'IA constitue le référentiel propre au niveau (comme la crèche). Chantier de **structure** (migration 3→6 + remap des matières via la procédure standard, jamais à la main). **Prouver sur le miroir ; jamais la vraie base sans GO.** | détail → [D68](BOUSSOLE/D68.md) |
...
| ⟳ | **66 — [Audit] Ce qui doit utiliser l'IA l'utilise vraiment** : passer en revue régulièrement toutes les fonctions d'aSchool censées décider via l'IA, et vérifier que chacune appelle réellement `generate()` — jamais du code en dur (liste de mots-clés, règle figée) qui **simule** l'intelligence. Née du cas `_age_est_flou` (détection d'ambiguïté qui devait être IA mais était du matching en dur). Toute fonction prise en flagrant délit = **défaut à corriger**. | **récurrent** — à relancer de temps en temps |
...
Rendre concret « dériver du référentiel », en deux briques : **(1)** un **script d'ingestion re-jouable** *référentiel → découpe → vecteurs → pgvector AVEC métadonnée `niveau`* — **livré** (`backend/rag/pgvector_store.py` + une **fiche par référentiel**, ex. `creche_0_3_ans.py`, niveau posé sur **chaque** chunk dès l'ingestion ; *le premier producteur BTS CIEL a été retiré le 08/07, à régénérer*) ; **(2)** un **harnais de tests** qui vérifie que `retrieve_pg()` remonte les bons extraits (`test_exemple_referentiel.py`, `test_decoupage_creche.py`) — **livré** (mesure la **qualité du RAG**, **pas** un « entraînement » du LLM — le Llama de Groq reste figé ; on alimente un référentiel, on ne ré-entraîne rien). **Granularité non négociable : un référentiel par couple matière+niveau** — jamais un corpus « cycle en bloc » qui mélange plusieurs programmes. **Reste : prouver l'apport du RAG sur UN couple** (une matière, sortie **avec vs sans** RAG) **avant d'élargir**. But structurel : relier les **deux référentiels qui ne se parlent pas** — (a) SQL/menus ↔ (b) pgvector/RAG.
