...
- [ ] Centraliser les réglages LLM (modèle, max_tokens, température, prompts) en base
     ...
     * **[ ] 4.1.c — max_tokens administrable** *(point de reprise — à attaquer en NOUVELLE session, à froid, pour être frais sur la décision de conception)* — rendre `max_tokens` administrable. **Effort MOYEN** (plus lourd que 4.1.b : rien de pré-écrit, éparpillé en **1 défaut + 6 surcharges par outil**, validation numérique **avec bornes + modale si hors-plage**).
       **AVANT tout constat/plan — trancher le fork de conception :** (question amont) max_tokens doit-il être **réglable par l'admin**, ou **rester un knob interne en code** ? — un mauvais réglage **TRONQUE les activités des profs** (risque différent du modèle). Puis, si administrable : **(a)** valeur **globale** · **(b)** valeur **par outil** (les 6) · **(c)** **hybride** (défaut global + surcharges).
       **Méthode habituelle :** discussion du fork → constat ciblé → plan → GO → code → preuve → validation → commit/push à ton ordre.
