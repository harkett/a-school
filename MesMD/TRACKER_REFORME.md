...
- [ ] Centraliser les réglages LLM (modèle, fournisseur, max_tokens, température, prompts) en base
     ...
     * **[ ] 4.1.e — fournisseur administrable (Groq ↔ Anthropic)** — sortir `AI_PROVIDER` du `.env` seul vers la base, pilotable depuis l'UI admin (**combo fermée** Groq / Anthropic, pas de saisie libre), pour basculer **sans éditer le `.env` ni redémarrer**. Même moule que le modèle (4.1.a/b) : clé `Setting` (ex. `ai_provider`) + résolveur `get_ai_provider(db)` lu en base + UI combo + **rechargeable à chaud**. **Constat 22/06 :** `AI_PROVIDER` lu uniquement en `.env` (`config.py:6`), routage `generator.py:23-30`, **aucune** clé en base, **aucun** sélecteur admin. **Liens à traiter :** un modèle appartient à un fournisseur (llama→Groq, claude→Anthropic) → couplage modèle↔fournisseur ; et l'adaptateur Anthropic **ignore la température** (`generator.py:142-143`) → lien avec 4.1.d. **Ordre 4.1.d / 4.1.e non figé.**
