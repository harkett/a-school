...
from backend.systeme.admin import get_ai_model, get_ai_provider, get_cle_texte, get_max_tokens, get_prompt
...
def api_optimize(
    ...
    try:
        raw = generate(prompt, cle=get_cle_texte(db), provider=get_ai_provider(db), model=get_ai_model(db), max_tokens=get_max_tokens(db, "optimiseur"), temperature=0, json_mode=True)
