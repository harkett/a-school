...
export default function AdminReferentiels() {
  ...
  // Cartouche « Ambiguïtés » (dernière étape) : les énoncés d'exemple de TOUT le référentiel,
  // écrits hors de l'application et recollés d'un bloc. Rien n'est payé ici — aucun appel d'IA.
  const [ambigOuvert, setAmbigOuvert] = useState(false)
  const [ambigEtat, setAmbigEtat] = useState(null)        // { total, ecrits, matieres:[…] }
  const [ambigPrompt, setAmbigPrompt] = useState(null)    // { niveau, total, prompt } de la fenêtre
  const [ambigColle, setAmbigColle] = useState('')        // le résultat, collé d'un bloc
  const [ambigBusy, setAmbigBusy] = useState(false)
  const [ambigCompte, setAmbigCompte] = useState(null)    // compte rendu du dernier collage
  const [ambigGenBusy, setAmbigGenBusy] = useState(false) // voie PAYANTE : appel en cours
  ...
  // ── Cartouche « Ambiguïtés » ───────────────────────────────────────────────────────────
  // Trois gestes, tous gratuits : lire l'état, prendre le prompt, recoller le résultat.
  const ambigQS = `cycle_id=${cycleId}&niveau=${encodeURIComponent(niveau || '')}`

  async function rafraichirAmbig() {
    if (!cycleId || !niveau) return
    try {
      const r = await fetchWithTimeout(`/api/admin/referentiels/ambiguites/etat?${ambigQS}`,
        { credentials: 'include' }, TIMEOUT_STD)
      setAmbigEtat(r.ok ? await r.json() : null)
    } catch { setAmbigEtat(null) }
  }

  async function ouvrirPromptAmbig() {
    try {
      // TIMEOUT_LONG : le prompt cherche, pour chaque matière, les passages du référentiel qui
      // la décrivent — le modèle d'embedding local met plusieurs secondes à se charger la
      // première fois. Aucun appel payé pour autant, tout se calcule sur la machine.
      const r = await fetchWithTimeout(`/api/admin/referentiels/ambiguites/prompt?${ambigQS}`,
        { credentials: 'include' }, TIMEOUT_LONG)
      const d = await r.json().catch(() => ({}))
      if (!r.ok) { showError(d.detail || 'Prompt indisponible pour ce référentiel.'); return }
      setAmbigPrompt(d)
    } catch { showError(MSG_TIMEOUT) }
  }

  async function collerAmbig() {
    if (!ambigColle.trim()) return
    setAmbigBusy(true)
    try {
      const r = await fetchWithTimeout('/api/admin/referentiels/ambiguites/coller', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ cycle_id: Number(cycleId), niveau, brut: ambigColle }),
      }, TIMEOUT_STD)
      const compte = await r.json().catch(() => ({}))
      if (!r.ok) { showError(compte.detail || 'Ce texte n’a pas pu être enregistré.'); return }
      setAmbigCompte(compte)
      // Rien n'a été écrit tant qu'un bloc n'a pas trouvé sa matière : on GARDE le texte collé,
      // l'admin corrige le nom et recolle. L'effacer lui ferait tout reprendre.
      if (compte.applique) setAmbigColle('')
      await rafraichirAmbig()
    } catch { showError(MSG_TIMEOUT) } finally { setAmbigBusy(false) }
  }

  // La VOIE PAYANTE de la cartouche Ambiguïtés : l'application appelle le moteur elle-même.
  // Le geste est encadré comme les autres clics payants de l'écran — on prévient AVANT, et le
  // rappel de la voie à 0 € est dans la boîte (option `payant`).
  async function genererAmbig() {
    const n = ambigEtat ? ambigEtat.total : 0
    const dejaEcrits = ambigEtat ? ambigEtat.ecrits : 0
    const ok = await demanderConfirmation({
      titre: 'ATTENTION — CE CLIC EST PAYANT',
      message:
        `Le prompt de ce référentiel va être envoyé au moteur d'IA, et cet appel est FACTURÉ.\n` +
        `Il porte ${n} matière${n > 1 ? 's' : ''} et les extraits du référentiel qui les décrivent : ` +
        `c'est l'appel le plus long de l'application, en entrée comme en sortie.\n` +
        (dejaEcrits > 0
          ? `\n${dejaEcrits} matière${dejaEcrits > 1 ? 's ont' : ' a'} déjà son énoncé : il sera REMPLACÉ.\n`
          : ''),
    ...
    setAmbigGenBusy(true)
    try {
      const r = await fetchWithTimeout('/api/admin/referentiels/ambiguites/generer', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      ...
      const compte = await r.json().catch(() => ({}))
      if (!r.ok) { showError(compte.detail || 'La génération n’a pas abouti.'); return }
      setAmbigCompte(compte)
      // L'appel a été payé : si un nom n'a pas été reconnu, sa réponse revient dans la zone de
      // collage plutôt que d'être perdue — l'admin corrige et recolle sans repayer.
      if (compte.brut) setAmbigColle(compte.brut)
      await rafraichirAmbig()
    } catch { showError(MSG_TIMEOUT) } finally { setAmbigGenBusy(false) }
  }
  ...
  useEffect(() => {
    ...
    // Les exemples d'ambiguïté du référentiel — lus en même temps que le reste : c'est eux qui
    // allument la pastille de la dernière cartouche, et le compte rendu d'un autre couple n'a
    // plus rien à faire à l'écran.
    fetchWithTimeout(`/api/admin/referentiels/ambiguites/etat?cycle_id=${cycleId}&niveau=${encodeURIComponent(niveau)}`,
      { credentials: 'include' }, TIMEOUT_STD)
      .then(r => (r.ok ? r.json() : null))
      .then(d => { if (annule) return; setAmbigEtat(d); setAmbigCompte(null); setAmbigColle('') })
      .catch(() => { if (!annule) setAmbigEtat(null) })
  ...
  const steps = [
    ...
    // Dernière étape : un énoncé d'exemple par matière, pour l'écran prof « Détecter les
    // ambiguïtés ». VERTE quand toutes les matières du référentiel ont le leur — une seule
    // suffirait à faire croire le travail fait.
    { id: 'ambiguites',    label: 'Ambiguïtés',        done: !!ambigEtat && ambigEtat.total > 0 && ambigEtat.ecrits === ambigEtat.total },
  ]
  ...
  return (
    <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
      ...
      {/* Carte 7 (DERNIÈRE étape) — AMBIGUÏTÉS : un énoncé d'exemple par matière, pour l'écran
          prof « Détecter les ambiguïtés ». Même aller-retour que les autres étapes de la voie
          « par vos soins », mais UNE FOIS pour tout le référentiel au lieu d'une fois par
          matière : le prompt porte toutes ses matières, et le résultat se recolle d'un bloc.
          Aucune IA n'est appelée d'ici — pas de badge IA, rien n'est payé. */}
      {estVisible('ambiguites') && (
        <div className="bg-white rounded-xl border border-gray-200 p-5 flex flex-col gap-4">
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10 }}>
            <h2 className="text-base font-semibold text-gray-800" style={{ margin: 0 }}>
              <Pastille etat={ambigEtat && ambigEtat.total > 0 && ambigEtat.ecrits === ambigEtat.total ? 'vert' : 'rouge'}
                titre="Vert = toutes les matières de ce référentiel ont leur énoncé d'exemple." />
              Ambiguïtés — énoncés d'exemple
              <span style={{ fontWeight: 400, color: '#94a3b8', marginLeft: 6, fontSize: 13 }}>
                ({ambigEtat ? `${ambigEtat.ecrits}/${ambigEtat.total} matière${ambigEtat.total > 1 ? 's' : ''}` : '…'})
              </span>
              <InfoGuide {...aideReferentiels('ambiguites')} />
            </h2>
            <button type="button" className="btn-secondary" style={{ fontSize: 12, padding: '4px 10px', whiteSpace: 'nowrap' }}
              title={ambigOuvert ? 'Réduire' : 'Développer'} onClick={() => setAmbigOuvert(o => !o)}>
              {ambigOuvert ? 'Réduire' : 'Développer'}
            </button>
          </div>

          {ambigOuvert && (<>
            <p style={{ margin: 0, fontSize: 13, color: '#475569', lineHeight: 1.6 }}>
              Cette étape écrit <strong>un énoncé d'exemple par matière</strong> : un vrai sujet de la
              matière, au niveau, avec des défauts glissés dedans. C'est ce que le professeur charge
              d'un clic, par « Utiliser un exemple », pour découvrir l'écran « Détecter les ambiguïtés ».
              Ils sont écrits une fois et rangés en base : il retrouve le même à chaque ouverture.
              <br />
              Deux façons de les obtenir, au choix — <strong>gratuitement</strong>, en prenant le prompt
              ci-dessous et en l'exécutant de votre côté, puis en recollant le résultat ; ou
              <strong> en payant</strong>, en laissant l'application appeler le moteur elle-même.
            </p>

            {/* L'état matière par matière : c'est lui qui dit le travail restant. Une liste des
                seuls exemples écrits cacherait précisément ce qui manque. */}
            {ambigEtat && ambigEtat.matieres.length > 0 && (
              <div className="flex flex-col gap-2">
                <label style={{ fontSize: 12, fontWeight: 700, color: '#475569' }}
                  title="Les matières de ce référentiel. Vert = son énoncé d’exemple est écrit. Rouge = il manque.">
                  Matières concernées
                </label>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                {ambigEtat.matieres.map(m => (
                  <span key={m.id}
                    title={m.ecrit ? 'Cette matière a son énoncé d’exemple' : 'Cette matière n’a pas encore d’énoncé d’exemple'}
                    style={{ fontSize: 11, fontWeight: 600, padding: '2px 8px', borderRadius: 5,
                      background: m.ecrit ? '#ecfdf5' : '#fef2f2',
                      color: m.ecrit ? '#065f46' : '#b91c1c',
                      border: '1px solid ' + (m.ecrit ? '#a7f3d0' : '#fecaca') }}>
                    {m.nom}
                  </span>
                ))}
                </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              <button type="button" className="btn-secondary"
                style={{ fontSize: 12, padding: '4px 10px', whiteSpace: 'nowrap' }}
                title="Ce prompt génère des exemples pour chacune des matières de ce référentiel. Ces exemples seront utilisés par les profs."
                onClick={ouvrirPromptAmbig}>
                <span aria-hidden="true" style={{ fontSize: 15, lineHeight: 1, marginRight: 2 }}>👁</span> Le prompt
              </button>
              <InfoGuide {...aideReferentiels('ambiguites_prompt')} />

              {/* ── Bloc de DROITE : le geste payant, seul, au bout de la ligne — comme dans les
                  cartouches Matières, Découpe et Types. Un bouton qui coûte ne se met pas en tête
                  de ligne, là où la main clique sans réfléchir. ── */}
              <div style={{ marginLeft: 'auto' }}>
                <button type="button" onClick={genererAmbig} disabled={ambigGenBusy}
                  title={ambigGenBusy ? 'Génération en cours…'
                    : 'PAYANT — l’application écrit elle-même les énoncés de toutes les matières de ce référentiel'}
                  style={{ display: 'inline-flex', alignItems: 'center', gap: 6, height: 32,
                    padding: '0 14px', borderRadius: 8, fontSize: 12.5, fontWeight: 700,
                    border: '1px solid ' + (ambigGenBusy ? '#e5e7eb' : '#6d28d9'),
                    background: ambigGenBusy ? '#f8fafc' : '#7c3aed',
                    color: ambigGenBusy ? '#94a3b8' : '#fff',
                    cursor: ambigGenBusy ? 'not-allowed' : 'pointer' }}>
                  <span aria-hidden="true">✨</span>
                  {ambigGenBusy ? 'Génération en cours…' : 'Générer les exemples'}
                  <span aria-hidden="true" title="Ce clic est facturé"
                    style={{ width: 18, height: 18, borderRadius: '50%', background: '#fbbf24',
                      color: '#78350f', fontSize: 11, fontWeight: 800, display: 'inline-flex',
                      alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>€</span>
                </button>
              </div>
            </div>

            {/* Après une génération PAYANTE réussie, l'admin n'a rien à coller : l'application a
                tout fait. Lui redemander un geste qui n'a plus lieu d'être le ferait douter de ce
                qu'il vient de voir. La zone reprend son sens quand la génération a buté sur un nom
                de matière : le texte payé y revient, et là il y a bien quelque chose à corriger. */}
            <div className="flex flex-col gap-2">
              <label style={{ fontSize: 12, fontWeight: 700, color: '#475569' }}>
                {ambigCompte && ambigCompte.applique && !ambigColle
                  ? 'Rien à coller — c’est fait'
                  : ambigColle && ambigCompte && !ambigCompte.applique
                    ? 'Corriger le résultat, puis réenregistrer'
                    : 'Coller le résultat entier'}
              </label>
              <textarea
                value={ambigColle}
                onChange={e => setAmbigColle(e.target.value)}
                placeholder={'Collez tout ce que le modèle vous a rendu, les blocs compris :\n\n### Nom de la matière\n=== ENONCE ===\n…\n\n=== DEFAUTS ===\n…'}
                className="w-full text-xs border border-gray-300 rounded p-2"
                style={{ minHeight: 160, fontFamily: 'inherit', lineHeight: 1.5, resize: 'vertical' }}
              />
              <div className="flex items-center gap-2 flex-wrap">
                <button type="button" onClick={collerAmbig} disabled={ambigBusy || !ambigColle.trim()}
                  className="btn-primary"
                  style={{ fontSize: 12, padding: '5px 12px',
                    cursor: (ambigBusy || !ambigColle.trim()) ? 'not-allowed' : 'pointer',
                    opacity: (ambigBusy || !ambigColle.trim()) ? 0.55 : 1 }}
                  title={ambigColle.trim() ? 'Répartir ces énoncés sur les matières de ce référentiel'
                    : 'Collez d’abord le résultat rendu par le modèle'}>
                  <span aria-hidden="true">✓</span>
                  {ambigBusy ? 'Enregistrement…' : 'Enregistrer'}
                </button>
                <span className="text-xs text-gray-400">
                  {ambigCompte && ambigCompte.applique && !ambigColle
                    ? 'Les énoncés sont en base. Cette zone ne sert qu’à la voie gratuite, ou à corriger un résultat refusé.'
                    : 'Chaque bloc part sur la matière que son titre nomme. Aucune n’est écrite si l’une d’elles n’est pas reconnue.'}
                </span>
              </div>
            </div>

            {/* Compte rendu du dernier collage — ce qui est entré, et surtout ce qui ne l'est pas. */}
            {ambigCompte && (
              <div style={{ fontSize: 12.5, lineHeight: 1.6, color: '#334155',
                background: ambigCompte.applique ? '#f0fdf4' : '#fef2f2',
                border: '1px solid ' + (ambigCompte.applique ? '#bbf7d0' : '#fecaca'),
                borderRadius: 8, padding: '10px 12px' }}>
                {ambigCompte.applique ? (
                  <div style={{ fontWeight: 700, color: '#166534' }}>
                    {ambigCompte.ecrits} ajouté{ambigCompte.ecrits > 1 ? 's' : ''}, {ambigCompte.remplaces} remplacé{ambigCompte.remplaces > 1 ? 's' : ''}.
                  </div>
                ) : (
                  <div style={{ fontWeight: 700, color: '#991b1b' }}>
                    Rien n'a été enregistré — corrigez ce qui suit et recollez.
                  </div>
                )}
                {ambigCompte.orphelins.length > 0 && (
                  <div style={{ marginTop: 6 }}>
                    <strong>Nom de matière inconnu dans ce référentiel</strong> (ils doivent être
                    écrits exactement comme au programme) :
                    <ul style={{ margin: '4px 0 0', paddingLeft: 18 }}>
                      {ambigCompte.orphelins.map(n => <li key={n}>{n}</li>)}
                    </ul>
                  </div>
                )}
                {ambigCompte.vides.length > 0 && (
                  <div style={{ marginTop: 6 }}>
                    <strong>Bloc sans énoncé</strong> :
                    <ul style={{ margin: '4px 0 0', paddingLeft: 18 }}>
                      {ambigCompte.vides.map(n => <li key={n}>{n}</li>)}
                    </ul>
                  </div>
                )}
                {ambigCompte.non_traitees.length > 0 && (
                  <div style={{ marginTop: 6 }}>
                    <strong>Déclarées non traitées</strong> — le modèle n'a pas su de quoi elles
                    parlent, leur couple reste vide plutôt que faux :
                    <ul style={{ margin: '4px 0 0', paddingLeft: 18 }}>
                      {ambigCompte.non_traitees.map(n => <li key={n}>{n}</li>)}
                    </ul>
                  </div>
                )}
                {ambigCompte.absentes.length > 0 && (
                  <div style={{ marginTop: 6 }}>
                    <strong>Absentes du texte collé</strong> — elles gardent ce qu'elles avaient :
                    <ul style={{ margin: '4px 0 0', paddingLeft: 18 }}>
                      {ambigCompte.absentes.map(n => <li key={n}>{n}</li>)}
                    </ul>
                  </div>
                )}
              </div>
      ...
      {/* Le prompt des exemples d'ambiguïté, à sélectionner puis copier — même geste que les
          autres fenêtres de prompt de cet écran. */}
      {ambigPrompt && (
        <div onClick={() => setAmbigPrompt(null)}
          style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.55)', zIndex: 2000,
            display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div onClick={e => e.stopPropagation()}
            style={{ background: '#fff', borderRadius: 12, width: '92%', maxWidth: 900, height: '82vh',
              display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 8px 32px rgba(0,0,0,0.3)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10,
              padding: '10px 14px', borderBottom: '1px solid #e2e8f0' }}>
              <span style={{ fontSize: 13, fontWeight: 700, color: '#1e293b' }}>
                Prompt des exemples — {ambigPrompt.niveau}
              </span>
              <span style={{ fontSize: 12, color: '#94a3b8' }}>
                {ambigPrompt.total} matière{ambigPrompt.total > 1 ? 's' : ''}
              </span>
              <button type="button" className="btn-secondary"
                style={{ marginLeft: 'auto', fontSize: 12, padding: '4px 10px', whiteSpace: 'nowrap' }}
                title="Sélectionner tout le texte de cette fenêtre — il ne reste plus qu'à le copier (Ctrl+C)"
                onClick={() => {
                  const n = document.getElementById('zone-prompt-ambiguites')
                  if (!n) return
              ...
              <button type="button" onClick={() => setAmbigPrompt(null)} title="Fermer"
                style={{ border: 'none', background: 'transparent', fontSize: 18, cursor: 'pointer',
                  color: '#64748b', lineHeight: 1 }}>✕</button>
            </div>
            {/* À QUOI SERT CE TEXTE, avant de le lire. On ouvrait cette fenêtre sans savoir si
                on tenait le prompt qui analyse l'énoncé du professeur ou celui qui écrit ses
                exemples : ils se ressemblent, et rien ne les distinguait à l'écran. */}
            <div style={{ fontSize: 12.5, lineHeight: 1.6, color: '#334155', background: '#f8fafc',
              borderBottom: '1px solid #e2e8f0', padding: '9px 14px' }}>
              Ce texte <strong>n'analyse rien</strong> : il écrit les énoncés d'exemple des matières
              ci-dessous, ceux que le professeur charge par « Utiliser un exemple ». Le prompt qui
              analyse son énoncé est un autre — Admin → IA → Prompts → Autres fonctionnalités.
            </div>

            {/* Le prompt ne demande QUE les matières que le référentiel décrit. Celles qu'il
                n'éclaire pas en sont absentes : le dire ici, sinon leur couple resterait vide
                sans que personne sache pourquoi. */}
            {ambigPrompt.sans_contexte.length > 0 && (
              <div style={{ fontSize: 12.5, lineHeight: 1.6, color: '#7c2d12', background: '#fff7ed',
                borderBottom: '1px solid #fed7aa', padding: '8px 14px' }}>
                <strong>Absentes de ce prompt</strong> — le découpage du référentiel ne dit rien de
                ces matières, on ne demande donc pas d'exemple qui serait deviné :
                <ul style={{ margin: '4px 0 0', paddingLeft: 18 }}>
                  {ambigPrompt.sans_contexte.map(n => <li key={n}>{n}</li>)}
                </ul>
              </div>
            )}
            <pre id="zone-prompt-ambiguites"
              style={{ flex: 1, overflow: 'auto', margin: 0, padding: 14, fontSize: 12,
                color: '#334155', whiteSpace: 'pre-wrap', background: '#f8fafc' }}>
              {ambigPrompt.prompt}
            </pre>
