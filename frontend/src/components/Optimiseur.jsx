...
export default function Optimiseur({ defaultMatiere, defaultNiveau, onNavigate }) {
  ...
  async function optimiser() {
    if (!sequence.trim()) {
      setErreur('Collez une séquence pédagogique avant de lancer l\'optimisation.')
      return
    }
    setErreur(null)
    setResultat(null)
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '16px 20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '6px' }}>
          <span style={{ color: 'var(--bordeaux)' }}><IconOptimiseur /></span>
      ...
      <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          {!resultat && (
            <button
              ...
              title="Pré-remplir avec une séquence fictive pour voir comment fonctionne l'analyse"
              disabled={loading}
              style={{ fontSize: '11px', color: '#6366f1', background: 'none', border: '1px solid #c7d2fe', borderRadius: '4px', padding: '3px 9px', cursor: 'pointer', fontWeight: 500 }}
            >
        ...
        {/* Bouton optimiser — bas à droite */}
        <div className="flex justify-end">
          <button
            className="btn-primary"
            onClick={optimiser}
            disabled={loading || !sequence.trim()}
            title={!sequence.trim() ? 'Collez une séquence pour lancer l\'analyse' : 'Lancer l\'analyse et l\'optimisation de la séquence'}
          >
            <IconOptimiser />
      ...
      {resultat && (
        <div ref={resultRef} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          {/* Avertissement incohérence matière */}
                ...
                {resultat.problemes.map((p, i) => {
                  ...
                  return (
                    <div key={i} style={{
                      ...
                      <span style={{ fontSize: '11px', fontWeight: 700, color: c.text, whiteSpace: 'nowrap', paddingTop: '1px', minWidth: '160px' }}>
                        {p.type}
                      </span>
