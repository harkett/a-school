...
5. **La preuve conclut le morceau — jamais de commit sans preuve.** Chaque morceau de la cadence ne se **committe** qu'après sa **preuve** : je l'**exécute** (étage 1) et je te **montre le résultat réel** (sortie de test, exécution) ; quand un geste admin/prof atteint le cas, **tu testes (étage 2) avant qu'on avance**. Si le morceau n'est **pas atteignable** (ex. une brique pas encore branchée), je le **dis clairement** (« non testé / non atteignable ») et on s'appuie sur l'étage 1 + les tests auto. **Aucun commit sans cette preuve.** *(Renforcé le 10/07/2026 : sans preuve à chaque morceau, je pars en vrille et je produis du code pas propre — l'historique le montre.)*
...
- **Garde-fou — pas de découpage sans règle validée.** La colonne `regle_valide` doit être `true` : la fiche **refuse d'ingérer** (lève) si la règle n'est pas validée (cap « aSchool n'invente rien »). La fiche lit le motif **de la base** (règle validée du couple), plus aucun regex en dur ni fichier.
...
Repère code : le socle passe la ligne `referentiels` du couple à la fiche (`fiche.charger_regle(ref)`) **avant** `extract_pages` ; la fiche compile le motif validé lu en base et pose l'arbitrage, ou lève si non validé (`backend/rag/referentiels/creche_0_3_ans.py`, `backend/rag/pgvector_store.py`). Endpoints admin (lire / valider / rejeter / aperçu + arbitrage) résolus par cycle + niveau via `_ref_du_couple` (`backend/pedagogie/referentiels_admin.py`).
...
C'est pour cela qu'on **ne touche jamais à la base** pour ce filtrage : **aucune colonne ajoutée** pour le **tri par niveau**. La règle « quel contenu appartient à quel niveau » vit dans le **code de la fiche** (elle reçoit le niveau en paramètre et ne renvoie que le bon contenu), **jamais dans la donnée**. Le niveau est déjà tenu de bout en bout du flux admin (l'admin choisit cycle + niveau au départ) : on le passe, on ne le stocke pas en plus. Corollaire : l'isolement par niveau est **structurel** (le mauvais contenu n'est même pas dans la collection), pas un filtre conditionnel qu'on pourrait oublier à la recherche.

> **À ne PAS confondre.** Ceci vaut UNIQUEMENT pour le **tri par niveau** (logique structurelle = code). Cela ne contredit en rien « tout en base » : la **règle de découpe** (motif, explication, statut), l'**arbitrage des cas flous**, le **seuil**, les **filtres** sont des **données métier** et vivent bien **en base** (colonnes de `referentiels`). « Aucune colonne » ne concerne que le filtrage structurel niveau→contenu, jamais un prétexte à garder une donnée en fichier.
