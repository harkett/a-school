...
### Admin — détail dans le code : `backend/routers/admin.py` + tables backoffice (§ Tables BDD ci-dessus). *(L'ancien `PLAN_BACKOFFICE_FASTAPI.md` n'existe plus — ne pas le citer.)*
...
### Auth + profil
- `users` — id, email, password_hash, is_verified, created_at, last_login, failed_attempts, locked_until, subject, prenom, nom, niveau, langue_lv, mobile, is_active
- `email_tokens` — id, token, email, user_id?, purpose (verify_email|reset_password), expires_at, used
- `refresh_tokens` — id, user_id, token_hash, expires_at, revoked
- `connexion_logs` — id, email, user_id?, action (signup|login), ip, created_at

### App
- `feedbacks` — id, user_id, type (feedback|notation), message, rating, category, statut, created_at, updated_at, attachment_path
- `settings` — key (PK), value
- `sequences_sauvegardees` — id, user_id, matiere, niveau, theme, duree, mode, description_classe, resultat, partagee, anonyme, created_at
- `activites_sauvegardees` — id, user_id, activite_key, activite_label, niveau, sous_type, nb, avec_correction, matiere, objet, partagee, anonyme, texte_source, resultat, created_at *(snapshot figé : niveau/matiere recopiés à la génération)*
- `feature_votes` — id, user_id, feature_key *(unique par paire)*
- `fiches_matieres` — id, matiere_key (unique), statut (brouillon|publie|a_reviser), accroche, pour_qui, ameliorations, updated_at, updated_by
- `tool_usage_logs` — id, user_id, tool (sequence|optimiseur), score_label, created_at
- `few_shot_milestones` — id, user_id, activite_key *(unique par paire)*, reached_at

### Backoffice
- `user_sessions` — id, user_id, session_key (unique), ip_address, user_agent, browser, os, device_type, login_at, last_seen, is_active
- `failed_login_attempts` — id, ip_address, username, user_agent, attempt_at, blocked
- `admin_audit_log` — id, admin_email, action, target_email, ip_address, details, timestamp
- `admin_alerts` — id, level, title, message, created_at, is_read, read_by, read_at

### Référentiel programmes (cycles/niveaux/matières)
- `cycles` — id, nom (unique), ordre, categorie (secondaire|superieur|creche|primaire|maternelle)
- `niveaux` — id, cycle_id, nom, ordre, traite
- `matieres` — id, cle (unique), nom, ordre, actif
- `matiere_niveaux` — id, matiere_id, niveau_id, actif *(paire valide = programme officiel)*
- `user_enseignements` — user_id + matiere_niveau_id (PK composite) — ce que le prof enseigne
